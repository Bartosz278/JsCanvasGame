...
export class Player {
  constructor(ctx, img, canvas,  interactiveObstacles, showCollectInfo, collectItem) {
    this.ctx = ctx;
    this.img = img;
    this.canvas = canvas;
  ...
  tryCollecting() {
    for (let i = this.interactiveObstacles.length - 1; i >= 0; i--) {
      ...
      if (distance < 50) {
        ...
        this.showCollectInfo(true, "Collecting...", this.x + 20, this.y - 20);
        setTimeout(() => {
          this.collectItem(i);
          this.isCollecting = false;
          this.showCollectInfo(false, "", this.x + 20, this.y - 20);
        }, obstacle.digTime);
        break;
      }
    }
