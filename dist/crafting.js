...
const craftingWindow = document.getElementById('craftingWindow');
...
function craftItem(craftableItem) {
    const canCraft = craftableItem.requiredItems.every((requiredItem) => {
        const inventoryItem = inventory.find((item) => item && item.name === requiredItem.name);
    ...
    craftableItem.requiredItems.forEach((requiredItem) => {
        const inventoryIndex = inventory.findIndex((item) => item && item.name === requiredItem.name);
        if (inventoryIndex !== -1) {
